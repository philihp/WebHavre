...
public class ThreePlayerLongGame {
	
	public static final GameMode MODE = GameMode.LONG;
	public static final PlayerCount COUNT = PlayerCount.THREE;
	...
	public void testShipDeck() {
		...
		assertEquals(ShipEnum.S03, deck[i++].identity);
		assertEquals(ShipEnum.S04, deck[i++].identity);
		assertEquals(ShipEnum.S05, deck[i++].identity);
		assertEquals(ShipEnum.S06, deck[i++].identity);
		assertEquals(ShipEnum.S07, deck[i++].identity);
		assertEquals(ShipEnum.S08, deck[i++].identity);
		assertEquals(ShipEnum.S10, deck[i++].identity);
		assertEquals(ShipEnum.S11, deck[i++].identity);
		assertEquals(ShipEnum.S12, deck[i++].identity);
		assertEquals(ShipEnum.S13, deck[i++].identity);
		assertEquals(ShipEnum.S14, deck[i++].identity);
		assertEquals(ShipEnum.S16, deck[i++].identity);
		assertEquals(ShipEnum.S17, deck[i++].identity);
		assertEquals(ShipEnum.S18, deck[i++].identity);
